...
function openWhatsApp(msg = "Hello, I'm interested in your products. Please share details.") {
...
function SubcategorySection({ sub, products }) {
  ...
  return (
    <div className="mb-16" id={`section-${sub.id}`}>
      <div className="flex items-end justify-between mb-6 px-6 max-w-7xl mx-auto">
        <div>
          <h2 className="text-2xl font-bold text-zinc-900 flex items-center gap-2">
            {sub.name}
            <span className="text-sm font-normal text-zinc-400">({products.length})</span>
          </h2>
        </div>
      ...
      <div ref={ref}
        ...
        {products.map(p => <ProductCard key={p.id ?? p.name} {...p} />)}
      </div>
    </div>
  )
}
...
// ── Main ──────────────────────────────────────────────────────────────────────
// Works for two cases:
//   1. /category/:id  — reads id from URL param
//   2. fixedName prop — looks up category by name (for /apparels, /drinkware, etc.)
export default function CategoryLandingPage({ fixedName } = {}) {
  const { id: urlId } = useParams()

  const [category,     setCategory]     = useState(null)
  const [subcategories,setSubcategories]= useState([])  // from Subcategories table
  // sections = [{ sub, products[] }]
  ...
  async function fetchAll() {
    ...
    // ── 2. Get subcategories from Subcategories table ────────────
    ...
    if (!allSubs.length) { setSections([]); setLoading(false); return }

    const subIds = allSubs.map(s => s.id)

    // ── 3. Get items from Items table ────────────────────────────
    ...
    if (!itemNames.length) { setSections([]); setLoading(false); return }

    // ── 4. Get products matching item names ──────────────────────
    ...
    // Set hero from first product with an image
    const firstWithImage = allProducts.find(p => p.image_url)
    if (firstWithImage) setHeroImage(firstWithImage.image_url)

    // ── 5. Group products by subcategory via Items ───────────────
    const productByName = {}
    allProducts.forEach(p => { productByName[p.name] = p })

    const built = allSubs.map(sub => {
      const subItems = allItems.filter(i => i.subcategory_id === sub.id)
      const subProducts = subItems
        .map(i => productByName[i.name])
        .filter(Boolean)
  ...
  if (notFound || !category) return (
    <><SideStrips /><Navbar />
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-6">
        ...
        <p className="text-sm text-zinc-500 mb-6">This category may have been removed or renamed.</p>
  ...
  return (
    <>
      ...
      <section className="relative w-full h-[50vh] min-h-[320px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-center px-8 md:px-20">
          ...
          <p className="text-white/80 text-sm md:text-base max-w-md mb-8">
            Custom printing on all products. Order as low as single quantity. Pan India delivery.
          </p>
          <button onClick={() => openWhatsApp(`Hello, I'm interested in ${category.name}. Please share details.`)}
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-1">
              {totalProducts} product{totalProducts !== 1 ? "s" : ""} · {subcategories.length} subcategor{subcategories.length !== 1 ? "ies" : "y"}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-6 mb-10">
          <div className="flex gap-6 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {subcategories.map(sub => {
              const firstImg = sections.find(s => s.sub.id === sub.id)?.products?.[0]?.image_url
              return (
                <a key={sub.id} href={`#section-${sub.id}`}
                  className="flex flex-col items-center gap-2 flex-shrink-0 group cursor-pointer">
                  <div className="w-20 h-20 rounded-full overflow-hidden ring-2 ring-zinc-100 group-hover:ring-[#5fc7f4] transition-all duration-300 bg-zinc-100">
                    {firstImg
                      ...
                      : <div className="w-full h-full flex items-center justify-center text-2xl font-bold text-white" style={{ background: `linear-gradient(135deg, ${DARK}, ${BRAND})` }}>
                          {sub.name.charAt(0)}
                        </div>}
                  </div>
      ...
      {sections.length === 0 ? (
        ...
        sections.map(({ sub, products }) => (
          <SubcategorySection key={sub.id} sub={sub} products={products} />
        ))
      )}
      ...
      <section className="mx-6 mb-16 rounded-3xl overflow-hidden" style={{ backgroundColor: DARK }}>
        <div className="px-10 py-12 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <button
            onClick={() => openWhatsApp(`Hello, I need a bulk quote for ${category.name}. Please share details.`)}
